...
- `retention` : le fichier doit-il être conservé (`required` vrai/faux), combien d'années (`years`, 0 si non requis), et sur quel fondement `basis` : `proof` (pièce probante : acte, procès-verbal, décision, contrat signé, facture, preuve d'un droit ou d'une obligation), `legal` (obligation légale de conservation), `fiscal` (pièce comptable ou fiscale, en général 10 ans), `rh` (dossier du personnel, paie, 5 ans après départ), `contractual` (durée du contrat + prescription, souvent 5 ans après la fin), `none` sinon. `justification` en une phrase, `confidence` 0–100. En cas de doute entre deux durées, retiens la plus longue.
