...
| `adaptive` (Alimentation adaptative) | faux | local | régule les **tokens en vol** au lieu d'envoyer `max_in_flight` blocs quoi qu'il arrive (`llm/pacer.py`) : montée par crans de +25 % tant que le débit (tokens rendus/s, fenêtres de 5 blocs) ne baisse pas, retour au cran d'avant et palier quand il baisse, descente si le serveur ralentit (autre usage du GPU), sonde de moins en moins fréquente en palier, budget divisé par deux sur détresse (coupure/délai après les tentatives, **préemptions vLLM** lues dans `/metrics` toutes les 30 s). Le budget trouvé est mémorisé par serveur + modèle (`pacer.json` dans le dossier de configuration) et sert de départ au run suivant. À préférer dès que le serveur est partagé ou change (5090 NVFP4+MTP vs 3090 FP8) ; le mode fixe reste le défaut, reproductible |
| `adaptive_start_tokens` | 0 | local | budget de départ du mode adaptatif ; 0 = mémorisé, sinon deux blocs (`blocks.block_tokens` × 2) |
